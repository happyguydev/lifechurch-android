...
public class ConversationMessagesHandler {
    ...
    static DatabaseReference blockDatabase;
    static Long status = null;
        ...
        if (conversationMessagesChildEventListener == null) {
            ...
            conversationMessagesChildEventListener = conversationMessagesNode.orderByChild(Message.TIMESTAMP_FIELD_KEY)
                ...
                public void onChildAdded(final DataSnapshot dataSnapshot, String prevChildKey) {
                    ...
                    try {
                        final Message message = decodeMessageSnapShop(dataSnapshot);
                        ...
                        if (message.getStatus() < Message.STATUS_RECEIVED_FROM_RECIPIENT_CLIENT
                            ...
                            blockDatabase = FirebaseDatabase.getInstance().getReference().child("/apps/" + ChatManager.Configuration.appId + "/BlockUser/" + message.getRecipient());
                            blockDatabase.addValueEventListener(new ValueEventListener() {
                                @Override
                                public void onDataChange(DataSnapshot dataSnapshot1) {
                                    if(dataSnapshot1.exists()){
                                       String status = dataSnapshot1.child(message.getSender()).child("blockStatus").getValue(String.class);
                                       Log.v("-->", status);
                                       if (status.equals("Message, Profile Blocked")) {
                                           return;
                                       } else {
...
//            blockDatabase = FirebaseDatabase.getInstance().getReference().child("/apps/" + ChatManager.Configuration.appId + "/BlockUser/" + (String) map.get("recipient"));
//            blockDatabase.addValueEventListener(new ValueEventListener() {
//                @Override
